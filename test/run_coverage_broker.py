...
"""
Build, run unit tests, and collect coverage for broker scope only.
...
Unit tests only:
    python test/run_coverage_broker.py --unit-only

Coverage only:
    python test/run_coverage_broker.py --coverage-only

Scoped coverage report (reuse existing profdata):
    python test/run_coverage_broker.py --scope src/topic/
...
SELECTION_FILE = TEST_DIR / "broker_tests.selection.txt"

THRESHOLD = 80.0

CLIENT_SOURCE_PREFIXES = (
    "src/client/",
    "src/client_api/",
    "src/test_client/",
    "src/yaha/",
...
def _open_log() -> None:
    ...
    _log_fh.write(
        f"=== run_coverage_broker.py run_id={_run_id} "
...
def _is_client_test_source(rel_path: str) -> bool:
    return rel_path.startswith(CLIENT_SOURCE_PREFIXES)


def _select_broker_tests(binary: Path) -> list[str]:
    discovered = _discover_tests(binary)
    selected = [name for name, rel in discovered if not _is_client_test_source(rel)]
    if not selected:
        print("\n[FAILED] no broker tests selected", file=sys.stderr)
        _close_log()
        sys.exit(1)
    return selected
...
def _default_coverage_targets() -> list[str]:
    excluded = {"client", "client_api", "test_client", "yaha"}
    targets = [
        str(path)
        for path in sorted(SRC_DIR.iterdir())
        if path.is_dir() and path.name not in excluded
    ]
    for file_name in ("main.cpp",):
...
def step_run_tests() -> str:
    selected = _select_broker_tests(DEBUG_BINARY)
    print(f"      selected broker tests: {len(selected)}")
    return _run_selected_tests("Broker unit tests", DEBUG_BINARY, selected, total_timeout_seconds=CTEST_TOTAL_TIMEOUT_SECONDS)
...
def step_collect_coverage() -> None:
    ...
    env = os.environ.copy()
    env["LLVM_PROFILE_FILE"] = str(COVERAGE_ARTIFACTS_DIR / "coverage-broker-%p.profraw")
    selected = _select_broker_tests(COV_BINARY)
    print(f"      selected broker tests: {len(selected)}")
    _run_selected_tests(
        "Broker coverage test execution",
        COV_BINARY,
        selected,
        env=env,
...
def print_summary(test_output: str, cov_output: str) -> None:
    ...
    if below:
        ...
        for item in below:
            print(f"    - {item}")
        print("\n  Use: python test/run_coverage_broker.py --show src/.../file.cpp")
    else:
        print(f"  Threshold  : MET  (all broker production files >= {THRESHOLD:.0f}%)")
...
def full_run() -> dict:
    ...
    print("[2/4] Running broker unit tests ...")
    ...
    print("[4/4] Measuring broker coverage ...")
...
def unit_only_run() -> dict:
    ...
    print("[2/2] Running broker unit tests ...")
    test_output = step_run_tests()
    passed, total = _parse_test_summary(test_output)
    print(f"      {passed}/{total} passed")
...
def coverage_only_run() -> dict:
    ...
    print("[2/2] Measuring broker coverage ...")
    step_collect_coverage()
    cov_output = step_coverage_report()
    print("      OK")
...
def _require_profdata() -> None:
    ...
    print(
        ...
        "Run 'python test/run_coverage_broker.py' first to build and collect coverage data.",
        file=sys.stderr,
    )
    sys.exit(1)
...
def main() -> None:
    ...
    group.add_argument("--unit-only", action="store_true", help="Run broker unit tests only.")
    group.add_argument("--coverage-only", action="store_true", help="Run broker coverage only.")
