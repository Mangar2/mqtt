...
- NEVER use `.inc` files as a workaround to satisfy line-limit goals.
- If refactoring is required for file size, split into domain-meaningful `.h`/`.cpp` objects.
- NOLINT is allowed only in C++ unit test source files (for example `src/**/test/*_test.cpp`)
- If clang-tidy reports diagnostics in C++ unit test source files, suppress them using the narrowest practical `NOLINTNEXTLINE` or `NOLINTBEGIN`/`NOLINTEND` block in that test file
...
- use std::fromat to format strings, never create strings otherwise if you concatenate more than two elements
