...
constexpr std::uint32_t sha1_rot_left(std::uint32_t val, std::uint32_t bits) noexcept {
...
std::array<std::uint8_t, 20> sha1(std::span<const std::uint8_t> data) {
    ...
    // Pre-processing: append 0x80, zero-pad to 56 mod 64, append 64-bit big-endian bit count.
    ...
    for (std::size_t blk = 0; blk < num_blocks; ++blk) {
        ...
        for (std::uint32_t idx = 0U; idx < 80U; ++idx) {
            ...
            const std::uint32_t tmp = sha1_rot_left(reg_a, 5U) + fun + reg_e + con + wrd[idx];
            reg_e = reg_d;
            reg_d = reg_c;
        ...
        hash[0] += reg_a; hash[1] += reg_b; hash[2] += reg_c;
        hash[3] += reg_d; hash[4] += reg_e;
    }

    std::array<std::uint8_t, 20> digest{};
    for (std::uint32_t idx = 0U; idx < 5U; ++idx) {
        digest[(idx * 4U)]      = static_cast<std::uint8_t>((hash[idx] >> 24U) & 0xFFU);
        digest[(idx * 4U) + 1U] = static_cast<std::uint8_t>((hash[idx] >> 16U) & 0xFFU);
        digest[(idx * 4U) + 2U] = static_cast<std::uint8_t>((hash[idx] >>  8U) & 0xFFU);
...
bool header_value_contains(std::string_view header_val, std::string_view token) noexcept {
    std::size_t pos = 0U;
...
bool WebSocketHandshake::is_complete() const noexcept {
    return complete_;
}

std::vector<uint8_t> WebSocketHandshake::build_response() const {
    if (!complete_) {
        throw std::logic_error("WebSocketHandshake::build_response called before handshake is complete");
    }
...
void WebSocketHandshake::parse_() {
    ...
    if (line_start == std::string_view::npos) {
        throw TransportException{TransportError::InvalidHandshake, "Malformed HTTP request"};
    }
    line_start += 2U;
    ...
    while (line_start < headers.size()) {
        ...
        if (iequals(name, "Upgrade") && iequals(value, "websocket")) {
            found_upgrade = true;
        } else if (iequals(name, "Connection") && header_value_contains(value, "Upgrade")) {
