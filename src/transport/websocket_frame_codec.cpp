...
bool is_known_opcode(std::uint8_t raw) noexcept {
    return raw == 0x0U || raw == 0x1U || raw == 0x2U ||
           raw == 0x8U || raw == 0x9U || raw == 0xAU;
}
...
WsFrame WebSocketFrameCodec::consume_frame() {
    if (frames_.empty()) {
        throw std::logic_error("WebSocketFrameCodec::consume_frame called with no frames available");
    }
...
void WebSocketFrameCodec::try_decode_() {
    while (true) {
        ...
        const std::size_t total_size = hdr.header_size + mask_size + hdr.payload_len;
        ...
        if (hdr.masked) {
            const std::array<std::uint8_t, 4> mask_key = {
                buf_[hdr.header_size],
                buf_[hdr.header_size + 1U],
                buf_[hdr.header_size + 2U],
                buf_[hdr.header_size + 3U]
            };
            ...
            const auto src_begin = buf_.begin() + static_cast<std::ptrdiff_t>(hdr.header_size);
            std::copy(src_begin, src_begin + static_cast<std::ptrdiff_t>(hdr.payload_len),
                      frm.payload.begin());
        }
...
std::vector<uint8_t> WebSocketFrameCodec::encode_frame_(
    bool fin,
    WsOpcode opcode,
    std::span<const uint8_t> payload)
{
    ...
    // Byte 1 (and optional extended length): MASK bit is never set for server frames.
    if (plen <= 125U) {
...
std::vector<uint8_t> WebSocketFrameCodec::encode_binary(std::span<const uint8_t> payload) {
    return encode_frame_(true, WsOpcode::Binary, payload);
}

std::vector<uint8_t> WebSocketFrameCodec::encode_control(
    WsOpcode opcode,
    std::span<const uint8_t> payload)
{
