...
| `client_api/`       | mqtt-client steps 23-24 | Public client facades: blocking `SyncClient` and non-blocking `AsyncClient` with completion callbacks and single-dispatch-thread callback delivery, while delegating protocol internals to `client/` components via callback integration. Depends on `client/` and `data_model/`. |
