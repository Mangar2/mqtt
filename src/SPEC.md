...
| `client_api/`       | mqtt-client steps 23-25 | Public client facades: blocking `SyncClient` and non-blocking `AsyncClient` with completion callbacks and single-dispatch-thread callback delivery, plus unified `ClientConfig` for transport/auth/session/reconnect/timeouts. Delegates protocol internals to `client/` components via callback integration. Depends on `client/` and `data_model/`. |
