...
| `yaha_msgstoreclient_main.cpp` | Standalone MessageStore runtime entry point with optional `<config-path>`, `--trace-messages`, `--version` (`-V`), and `--help`; prints versioned startup summary, logs broker connect/subscription lifecycle via MQTT runtime, and supports incoming-only broker-format message logs via INI key `[messagestore] logIncomingMessages=true` |
