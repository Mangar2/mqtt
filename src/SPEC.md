...
| `test_client/`      | mqtt-client step 27 | Standalone test-client shell module: persistent connection profiles, CLI parsing (`connect`, `save-profile`, `show-profile`), MQTT 5.0 only connection envelope (`mqtt`/`ws`, no TLS), and connect-session orchestration used by the `yahatestclient` executable. |
