...
| `test_client/`      | mqtt-client steps 27-32 | Standalone test-client shell module: persistent connection profiles, CLI parsing (`connect`, `publish`, `subscribe`, `scenario`, `save-profile`, `show-profile`), MQTT 5.0 only connection envelope (`mqtt`/`ws`, no TLS), full MQTT 5 CONNECT property and will coverage, one-shot publish matrix, subscribe/output pipeline controls, built-in scripted scenario catalog/runner, and Step 32 load-mode execution with machine-readable metrics used by the `yahatestclient` executable. |
