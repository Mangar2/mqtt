...
| `test_client/`      | mqtt-client steps 27-30 | Standalone test-client shell module: persistent connection profiles, CLI parsing (`connect`, `publish`, `subscribe`, `save-profile`, `show-profile`), MQTT 5.0 only connection envelope (`mqtt`/`ws`, no TLS), full MQTT 5 CONNECT property and will coverage, one-shot publish matrix, and subscribe/output pipeline controls used by the `yahatestclient` executable. |
