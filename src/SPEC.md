...
| `yaha_zwaveclient_main.cpp` | Standalone ZWave runtime entry point with optional `<config-path>`, `--trace-messages`, and `--help`; loads ZWave + MQTT config, composes `ZwaveController` + `ZwaveServiceComponent` with generic `YahaMqttClient`, and runs via `YahaMqttClientRuntime` until shutdown signal |
