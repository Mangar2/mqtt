...
| `svc/`              | operations helper | Shell helper command for systemd operations. Current subcommands: `svc list [pattern]` for PM2-like system-service overview (state, restart count, memory, PID, and last start timestamp) defaulting to the deployment-managed units `broker`, `filestore`, `msgstore`, `autom`, `valuesvc`, `serialdev`, `brkconn`, `httpmqtt`, and `remotesvc`, plus `svc log <service> [lines]` for journal following in the service-specific journal namespace with short unit names accepted. |
| `json/`             | shared utility | Reusable JSON value module with JavaScript-like dynamic object/array access, recursive parser (`parse`/`try_parse`) and compact serializer (`stringify`) for cross-module YAHA and broker usage. |
| `helper/`           | shared utility | Generic, domain-independent string helpers (`toLower`, `trim`, `split`) consolidating free-function duplicates previously reimplemented across broker and YAHA modules. See `helper/SPEC.md`. |

| `yaha/`             | yaha step 1+ | YAHA home automation client components including shared `error_handling/` (`YahaError`) for unified throw and API error outputs, shared `message/` value-type plus canonical envelope parser/builder utilities, automation tokenizer, standalone HTTP MQTT interface runtime wiring with ACK-gated broker-forward publish logging and explicit broker publish error logging in `http_mqtt_interface_client/`, ValueService phase-2/phase-3 modules in `value_service/` and `value_service_client/`, OpenSenseMap phase-1/phase-2 modules in `opensensemap/` and `opensensemap_client/` with repeated `[sensor]` section mapping plus per-sensor `minUploadIntervalSeconds` upload guard, Pushover phase-1/phase-2 modules in `pushover/` and `pushover_client/` with repeated `[device]` and `[subscription]` mapping, RS485 phase-6 runtime with config/composition (`rs485_interface_client/`) plus serial protocol/mapper/component/state modules in `rs485_protocol/`, `rs485_interface/`, and `rs485_state/` including legacy parity and component/runtime verification tests, SerialDevice phase-4 domain/parser/serializer/mapper/runtime modules in `serial_device/` plus phase-5 standalone config/composition/runtime-wiring modules in `serial_device_client/`, and MessageStore HTTP time output alignment to ISO-8601 UTC strings plus internal TreeNode-owned StringDirectory storage while MessageTreeNode remains a DTO-only output type. No source files at this level; each sub-topic has its own subdirectory. See `yaha/SPEC.md`. |
...
| `yaha_opensensemapclient_main.cpp` | Standalone OpenSenseMap runtime entry point with optional `<config-path>`, `--trace-messages`, and `--help`; loads OpenSenseMap + MQTT config, composes `OpenSenseMapComponent` with HTTP sender callback, and runs via `YahaMqttClientRuntime` until shutdown signal |
| `yaha_pushoverclient_main.cpp` | Standalone Pushover runtime entry point with optional `<config-path>`, `--trace-messages`, and `--help`; loads Pushover + MQTT config, composes `PushoverComponent` with HTTPS sender callback, and runs via `YahaMqttClientRuntime` until shutdown signal |
| `yaha_rs485interfaceclient_main.cpp` | Standalone RS485 interface runtime entry point with optional `<config-path>`, `--trace-messages`, and `--help`; loads RS485 + MQTT config, composes runtime objects via rs485 client composition helper, and runs via `YahaMqttClientRuntime` until shutdown signal |
| `yaha_remoteserviceclient_main.cpp` | Standalone RemoteService runtime entry point with optional `<config-path>`, `--trace-messages`, and `--help`; loads RemoteService + MQTT config, composes `RemoteServiceComponent` + `RemoteServiceHttpAdapter` + `YahaMqttClient`, starts dynamic HTTP GET/POST routing, and runs until shutdown signal with clean stop order |
| `yaha_zwaveclient_main.cpp` | Standalone ZWave runtime entry point with optional `<config-path>`, `--trace-messages`, and `--help`; loads ZWave + MQTT config (including optional `[zwave] logIncomingMessages/logOutgoingMessages`), composes `OpenZwaveRuntimeDriverPort` + `ZwaveController` + `ZwaveServiceComponent` with generic `YahaMqttClient`, and runs via `YahaMqttClientRuntime` until shutdown signal |
| `yaha_msgstoreclient_main.cpp` | Standalone MessageStore runtime entry point with optional `<config-path>`, `--trace-messages`, `--version` (`-V`), and `--help`; prints versioned startup summary, logs broker connect/subscription lifecycle via MQTT runtime, and supports incoming-only broker-format message logs via INI key `[messagestore] logIncomingMessages=true` with optional plain reason string output controlled by `[messagestore] logReason` |
