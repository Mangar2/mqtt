...
| `yaha_rs485interfaceclient_main.cpp` | Standalone RS485 interface runtime entry point with optional `<config-path>`, `--trace-messages`, and `--help`; loads RS485 + MQTT config, composes `Rs485InterfaceComponent` + `Rs485SerialAdapter` + `YahaMqttClient`, opens configured serial interface, and runs via `YahaMqttClientRuntime` until shutdown signal |
...
| `yaha_zwaveclient_main.cpp` | Standalone ZWave runtime entry point with optional `<config-path>`, `--trace-messages`, and `--help`; loads ZWave + MQTT config, composes `OpenZwaveRuntimeDriverPort` + `ZwaveController` + `ZwaveServiceComponent` with generic `YahaMqttClient`, and runs via `YahaMqttClientRuntime` until shutdown signal |
