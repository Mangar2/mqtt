...
| `client_api/`       | mqtt-client step 23 | Blocking public client facade exposing `connect`, `publish`, `subscribe`, `unsubscribe`, and `disconnect`, while delegating protocol internals to `client/` components via callback integration. Depends on `client/` and `data_model/`. |
