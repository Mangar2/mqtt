...
| `test_client/`      | mqtt-client steps 27-29 | Standalone test-client shell module: persistent connection profiles, CLI parsing (`connect`, `publish`, `save-profile`, `show-profile`), MQTT 5.0 only connection envelope (`mqtt`/`ws`, no TLS), full MQTT 5 CONNECT property and will coverage, and one-shot publish matrix with payload source/encoding/property controls used by the `yahatestclient` executable. |
