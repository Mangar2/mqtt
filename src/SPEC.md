...
| `test_client/`      | mqtt-client steps 27-31 | Standalone test-client shell module: persistent connection profiles, CLI parsing (`connect`, `publish`, `subscribe`, `scenario`, `save-profile`, `show-profile`), MQTT 5.0 only connection envelope (`mqtt`/`ws`, no TLS), full MQTT 5 CONNECT property and will coverage, one-shot publish matrix, subscribe/output pipeline controls, and built-in scripted scenario catalog/runner used by the `yahatestclient` executable. |
