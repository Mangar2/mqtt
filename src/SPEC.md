...
| `client/`           | mqtt-client steps 16-20 | Client-side components for outbound operation: active keep-alive manager, outbound topic-alias manager, CONNECT/CONNACK connection negotiator, session-state keeper for reconnect restore, and subscription manager for SUBSCRIBE/UNSUBSCRIBE plus inbound callback dispatch. Depends on `codec/`, `data_model/`, `network/`, `store/`, `topic/`. |
