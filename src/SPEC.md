...
| `client/`           | mqtt-client steps 16-21 | Client-side components for outbound operation: active keep-alive manager, outbound topic-alias manager, CONNECT/CONNACK connection negotiator, session-state keeper for reconnect restore, subscription manager for SUBSCRIBE/UNSUBSCRIBE plus inbound callback dispatch, and publish pipeline for outbound QoS tracking. Depends on `codec/`, `data_model/`, `network/`, `store/`, `topic/`, `qos/`. |
