...
TEST_CASE("takeover_handler_unregister_noop_when_not_active", "[session_manager]") {
    SessionTakeoverHandler handler;
    CHECK_NOTHROW(handler.unregister_connection("unknown"));
}

TEST_CASE("takeover_handler_takeover_returns_false_when_not_active", "[session_manager]") {
    SessionTakeoverHandler handler;
    CHECK_FALSE(handler.takeover_if_exists("c1"));
}

TEST_CASE("takeover_handler_takeover_invokes_callback_and_returns_true", "[session_manager]") {
    SessionTakeoverHandler handler;
    bool closed = false;
...
TEST_CASE("takeover_handler_size_tracks_active_connections", "[session_manager]") {
    SessionTakeoverHandler handler;
    CHECK(handler.size() == 0U);
...
TEST_CASE("expiry_scheduler_zero_interval_always_expired", "[session_manager]") {
    SessionExpiryScheduler scheduler;
...
TEST_CASE("expiry_scheduler_normal_interval_expired_after_deadline", "[session_manager]") {
    SessionExpiryScheduler scheduler;
...
TEST_CASE("expiry_scheduler_normal_interval_not_expired_before_deadline", "[session_manager]") {
    SessionExpiryScheduler scheduler;
    scheduler.schedule("c1", k_epoch, 60U);
...
TEST_CASE("session_manager_clean_start_creates_new_session", "[session_manager]") {
    SessionManagerFixture fix;
    const auto result =
...
TEST_CASE("session_manager_clean_start_removes_existing_session", "[session_manager]") {
    SessionManagerFixture fix;
    ...
    const auto result =
        fix.manager.handle_connect(make_connect("c1", true), []() {});
    CHECK_FALSE(result.session_present);
    // A fresh session has been created (expiry from CONNECT = 0 since no property set).
...
TEST_CASE("session_manager_takeover_invokes_close_callback", "[session_manager]") {
    SessionManagerFixture fix;
...
TEST_CASE("session_manager_disconnect_zero_expiry_removes_session", "[session_manager]") {
    SessionManagerFixture fix;
...
TEST_CASE("session_manager_disconnect_nonzero_expiry_schedules_timer", "[session_manager]") {
    SessionManagerFixture fix;
...
TEST_CASE("session_manager_disconnect_expiry_override", "[session_manager]") {
    SessionManagerFixture fix;
    // Session stored with expiry=60, but DISCONNECT overrides to 0 (immediate drop).
    fix.manager.handle_connect(make_connect("c1", true, 60U), []() {});
    fix.manager.handle_disconnect("c1", std::optional<uint32_t>{0U}, k_epoch);
    CHECK_FALSE(fix.session_store.contains("c1"));
}

TEST_CASE("session_manager_cleanup_expired_removes_sessions", "[session_manager]") {
    SessionManagerFixture fix;
    ...
    const auto cleaned =
        fix.manager.cleanup_expired(k_epoch + 30s);
...
TEST_CASE("session_manager_cleanup_expired_cancels_timer", "[session_manager]") {
    SessionManagerFixture fix;
