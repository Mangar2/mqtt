 ...
 * All decode functions consume bytes from a `ReadBuffer`; the cursor advances past
 * decoded data.
 */
...
/**
 * @brief Encodes a Two Byte Integer in big-endian order and appends 2 bytes to @p buf.
 * @param buf   Destination buffer.
...
/**
 * @brief Encodes a Four Byte Integer in big-endian order and appends 4 bytes to @p buf.
 * @param buf   Destination buffer.
 * @param value Value to encode.
 */
...
/**
 * @brief Encodes a UTF-8 string (2-byte big-endian length prefix + UTF-8 bytes) to @p buf.
 * @param buf Destination buffer.
...
/**
 * @brief Encodes Binary Data (2-byte big-endian length prefix + raw bytes) to @p buf.
 * @param buf  Destination buffer.
 ...
 * @throws CodecException(VariableByteIntegerOverflow) if the continuation bit is set in byte 4.
 */
[[nodiscard]] VariableByteInteger decode_variable_byte_integer(ReadBuffer& buf);

/**
 * @brief Decodes a Two Byte Integer (big-endian) from @p buf, consuming 2 bytes.
 * @param buf Source buffer.
...
/**
 * @brief Decodes a Four Byte Integer (big-endian) from @p buf, consuming 4 bytes.
 * @param buf Source buffer.
...
/**
 * @brief Decodes a UTF-8 string pair (name string followed by value string) from @p buf.
 * @param buf Source buffer.
