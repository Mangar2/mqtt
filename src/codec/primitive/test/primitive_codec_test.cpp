...
{
    std::vector<uint8_t> data{0xFFU, 0xFFU, 0xFFU, 0xFFU}; // continuation in byte 4
    auto reader = make_reader(data);
    try {
        (void)decode_variable_byte_integer(reader);
        FAIL("Expected CodecException");
...
{
    for (uint32_t v : {0U, 1U, 127U, 128U, 16383U, 16384U, 2097151U, 2097152U, 268435455U}) {
        WriteBuffer buf;
        encode_variable_byte_integer(buf, VariableByteInteger{v});
        auto reader = make_reader(buf);
        CHECK(decode_variable_byte_integer(reader).value == v);
    }
}
...
{
    std::vector<uint8_t> data{0x00U, 0x04U, 0x01U}; // says 4 bytes, only 1 available
    auto reader = make_reader(data);
    try {
