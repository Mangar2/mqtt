...
void encode_byte(WriteBuffer &buf, uint8_t byte) { buf.push_back(byte); }
...
uint8_t decode_byte(ReadBuffer &buf) { return buf.read_byte(); }
...
TwoByteInteger decode_two_byte_integer(ReadBuffer &buf) {
  ...
  return static_cast<TwoByteInteger>((static_cast<uint16_t>(bytes[0]) << 8U) |
