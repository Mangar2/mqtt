...
TEST_CASE("fh_decode_connect", "[fixed_header]")
{
    ...
    auto fh = decode_fixed_header(reader);
    CHECK(fh.type             == PacketType::Connect);
    CHECK(fh.flags            == 0x00U);
    CHECK(fh.remaining_length == 10U);
}
...
TEST_CASE("fh_decode_publish_flags", "[fixed_header]")
{
    ...
    auto reader = make_reader(data);
    auto fh = decode_fixed_header(reader);
    CHECK(fh.type             == PacketType::Publish);
    CHECK(fh.flags            == 0x0AU);
    CHECK(fh.remaining_length == 20U);
}
...
TEST_CASE("fh_decode_subscribe", "[fixed_header]")
{
    ...
    auto reader = make_reader(data);
    auto fh = decode_fixed_header(reader);
    CHECK(fh.type             == PacketType::Subscribe);
    CHECK(fh.flags            == 0x02U);
    CHECK(fh.remaining_length == 0U);
}
...
TEST_CASE("fh_decode_pubrel", "[fixed_header]")
{
    ...
    auto reader = make_reader(data);
    auto fh = decode_fixed_header(reader);
    CHECK(fh.type  == PacketType::Pubrel);
    CHECK(fh.flags == 0x02U);
}
...
TEST_CASE("fh_decode_unsubscribe", "[fixed_header]")
{
    ...
    auto reader = make_reader(data);
    auto fh = decode_fixed_header(reader);
    CHECK(fh.type  == PacketType::Unsubscribe);
    CHECK(fh.flags == 0x02U);
}
...
TEST_CASE("fh_decode_multi_byte_remaining", "[fixed_header]")
{
    ...
    auto reader = make_reader(data);
    auto fh = decode_fixed_header(reader);
    CHECK(fh.type             == PacketType::Connack);
    CHECK(fh.remaining_length == 128U);
}
...
{
    std::vector<uint8_t> data{0x81U, 0x00U}; // SUBSCRIBE with flags=0x01 (must be 0x02)
    auto reader = make_reader(data);
    try {
        (void)decode_fixed_header(reader);
        FAIL("Expected CodecException");
...
{
    std::vector<uint8_t> data{0x60U, 0x00U}; // PUBREL with flags=0x00 (must be 0x02)
    auto reader = make_reader(data);
    try {
        (void)decode_fixed_header(reader);
        FAIL("Expected CodecException");
...
{
    std::vector<uint8_t> data{0x10U}; // CONNECT byte only, no VBI remaining length
    auto reader = make_reader(data);
    try {
        (void)decode_fixed_header(reader);
        FAIL("Expected CodecException");
...
{
    for (auto& hdr : std::initializer_list<FixedHeader>{
