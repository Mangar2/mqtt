 ...
 * @brief MQTT 5.0 SUBSCRIBE, SUBACK, UNSUBSCRIBE, and UNSUBACK packet structs (Sections 3.8–3.11).
 */
...
struct SubscribeFilter {
    ...
    SubscriptionOptions options;       ///< Subscription options for this filter.
...
struct SubscribePacket {
    ...
    std::vector<SubscribeFilter> filters;   ///< At least one filter required by the spec.
...
struct SubackPacket {
    uint16_t                 packet_id{0};
    std::vector<Property>    properties;
    std::vector<ReasonCode>  reason_codes;  ///< One result code per SUBSCRIBE filter.
...
struct UnsubscribePacket {
    uint16_t                 packet_id{0};
    std::vector<Property>    properties;
    std::vector<Utf8String>  topic_filters;  ///< At least one filter required by the spec.
 ...
 *
 * @p reason_codes contains one entry per filter in the corresponding UNSUBSCRIBE.
 */
struct UnsubackPacket {
    uint16_t                 packet_id{0};
    std::vector<Property>    properties;
    std::vector<ReasonCode>  reason_codes;  ///< One result code per UNSUBSCRIBE filter.
