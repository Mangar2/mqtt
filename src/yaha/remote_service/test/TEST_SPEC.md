...
| `remote_service_runtime_config_falls_back_on_invalid_subscribe_qos` | invalid QoS parser path | ini with `remoteservice.subscribeQoS=7` | load succeeds and keeps default subscribe QoS |
| `remote_service_runtime_config_falls_back_on_invalid_filestore_port` | invalid port parser path | ini with `filestore.port=70000` | load succeeds and keeps default FileStore port |
| `remote_service_runtime_config_parses_message_logging_flags` | remoteservice logging config parsing | ini with `remoteservice.logIncomingMessages=true`, `remoteservice.logOutgoingMessages=true`, `remoteservice.logReason=true` | runtime config enables incoming/outgoing logging and reason-chain logging, and `remoteServiceConfig.logOutgoingMessages` mirrors `logOutgoingMessages` |
| `remote_service_runtime_config_falls_back_on_invalid_message_logging_flags` | invalid remoteservice logging flags fallback | ini with `remoteservice.logIncomingMessages=maybe`, `remoteservice.logOutgoingMessages=maybe`, `remoteservice.logReason=maybe` | runtime config keeps defaults and parsing still succeeds |
| `remote_service_runtime_config_ignores_mqtt_log_reason` | mqtt logReason legacy compatibility | ini with `mqtt.logReason=false` and no `remoteservice.logReason` | runtime config keeps reason-chain logging enabled via remoteservice defaults |
...
| `remote_service_component_logs_outgoing_message_when_enabled` | Message-flow logging for successfully published commands via shared `buildMessageLogLine` | `logOutgoingMessages=true`, successful `publishCommand` | stdout contains `remote_service -> <topic> :` for the mapped topic (asserted via `yaha::test::messageLogLinePrefix`) |
| `remote_service_component_does_not_log_when_disabled` | Logging stays silent by default | default `RemoteServiceConfig` (`logOutgoingMessages=false`), successful `publishCommand` | stdout contains no `remote_service` log line |
...
| `remote_service_component_publish_command_logs_failure_when_callback_missing` | callback-missing observability | loaded mapping, valid request, no callback set | structured `remote_service[error] op=publish_command reason=callback_missing` is logged |
| `remote_service_component_publish_command_logs_failure_when_callback_returns_error` | callback-result observability | loaded mapping, valid request, callback returns failed `PublishResult` | structured `remote_service[error] op=publish_command reason=publish_rejected` with category/reason is logged |
...
| `remote_service_component_publish_command_logs_failure_when_callback_throws` | callback-exception observability | loaded mapping, valid request, callback throws | structured `remote_service[error] op=publish_command reason=exception` is logged |