...
| `remote_service_component_logs_outgoing_message_when_enabled` | Message-flow logging for successfully published commands via shared `buildMessageLogLine` | `logOutgoingMessages=true`, successful `publishCommand` | stdout contains `component="remote_service" direction="outgoing"` for the mapped topic |
| `remote_service_component_does_not_log_when_disabled` | Logging stays silent by default | default `RemoteServiceConfig` (`logOutgoingMessages=false`), successful `publishCommand` | stdout contains no `component="remote_service"` log line |
