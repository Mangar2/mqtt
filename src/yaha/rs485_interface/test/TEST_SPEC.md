...
| `rs485_interface_component_trace_messages_logs_non_internal_with_legacy_format` | trace-level `messages` should log only non-internal serial frames in legacy JS format | component trace `messages`, one enable-send token and one normal serial frame | output contains exactly one legacy-formatted line (`sender => receiver (r:x): cmd = value ([len]  xx ..)`) |
| `rs485_interface_component_trace_internal_logs_token_and_non_token` | trace-level `internal` should include token and non-token serial frames | component trace `internal`, one enable-send token and one normal serial frame | output contains both legacy-formatted token and non-token lines |
...
| `rs485_interface_component_logs_incoming_message_when_enabled` | Message-flow logging for inbound MQTT messages via shared `buildMessageLogLine` | `logIncomingMessages=true`, `handleMessage` called with a message | stdout contains deterministic log line `component="rs485_interface" direction="incoming" topic="..." value=... qos=... retain=... dup=... reason=...` |
| `rs485_interface_component_logs_outgoing_message_when_enabled` | Message-flow logging for outbound MQTT messages via shared `buildMessageLogLine` | `logOutgoingMessages=true`, inbound serial frame triggers a mapped publish | stdout contains deterministic outgoing log line for the published message |
| `rs485_interface_component_does_not_log_when_disabled` | Logging stays silent by default | default config (`logIncomingMessages=false`, `logOutgoingMessages=false`), `handleMessage` called | stdout contains no `component="rs485_interface"` log line |
