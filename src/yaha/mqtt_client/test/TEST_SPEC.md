...
| `broker_transport_connect_poll_publish_and_unsubscribe_roundtrip` | Default broker transport callback bundle performs packet roundtrip with a broker | local fake MQTT broker with connack/suback/unsuback and incoming pingresp/pubrel/publish packets including forwarded JSON payload envelope | connect/subscribe/poll/publish/ping/unsubscribe/disconnect complete, inbound forwarded payload text is forwarded unchanged (no reconstruction), raw payload is preserved, and outbound publish uses raw payload bytes unchanged when present |
