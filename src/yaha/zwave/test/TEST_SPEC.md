...
| `logging_flags_emit_incoming_and_outgoing_lines` | Optional inbound/outbound trace logs are controlled by config flags | config with `logIncomingMessages=true` and `logOutgoingMessages=true`, one inbound set and one outbound controller publish | stdout contains one `zwave_service[in]` line and one `zwave_service[out]` line |
| `log_level_one_emits_important_events_without_forcing_message_traces` | log level 1 emits important events but does not force MQTT trace logs | config with `logLevel=1` and both trace flags disabled, startup + addnode management message | stdout contains `zwave_service[event]` lines for startup/addnode and contains no `zwave_service[in|out]` lines |
