...
| `load_zwave_config_applies_unified_log_level` | Unified log level drives message log toggles | `[zwave]` with `logLevel=3` and explicit false message-log flags | parse success; log level is `3`; incoming/outgoing message logs are enabled |
| `load_zwave_config_disables_message_logs_for_log_level_zero` | Unified log level zero disables protocol/message logging path | `[zwave]` with `logLevel=0` and explicit true message-log flags | parse success; log level is `0`; incoming/outgoing message logs are disabled |
