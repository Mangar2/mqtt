...
| `load_zwave_config_parses_poll_interval_ms_and_rejects_zero` | Poll interval setting is parsed with wide int64 range and non-zero minimum | valid `pollIntervalMs=750` and invalid `pollIntervalMs=0` | valid config sets `pollIntervalMs`; invalid config fails and references `zwave.pollIntervalMs` |
| `load_zwave_config_parses_command_reaction_timing_and_rejects_zero` | Command reaction poll/timeout settings are parsed with wide int64 range and non-zero minimum | valid `commandReactionPollIntervalMs=200` and `commandReactionTimeoutMs=30000`, invalid `commandReactionTimeoutMs=0` | valid config sets both fields; invalid config fails and references `zwave.commandReactionTimeoutMs` |
