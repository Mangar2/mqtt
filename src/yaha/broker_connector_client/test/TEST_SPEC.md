...
| `load_runtime_config_rejects_invalid_monitoring_log_incoming` | Monitoring section incoming-log bool validation failure | config with `monitoring.logIncomingMessage=maybe` | parser returns false and reports monitoring bool error |
...
| `load_runtime_config_falls_back_on_invalid_structured_subscription_shape` | structured subscription fallback path | `subscription.qos` appears without preceding `subscription.topic` | parser succeeds and falls back to default `#` qos1 subscription |
| `load_runtime_config_falls_back_on_invalid_receiver_timing_values` | receiver timing fallback path | invalid optional receiver keepalive/reconnect/loopsleep values | parser succeeds and defaults are retained |
