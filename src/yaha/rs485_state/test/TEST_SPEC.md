...
| `rs485_token_exchange_setters_and_state_info_are_observable` | token exchange exposes setter/getter and state info diagnostics | direct setVersion and setMaySend calls on fresh exchange | version and may-send flags are applied and state info string contains all expected sections |
| `rs485_token_exchange_updates_leftmost_and_right_sibling_paths` | token exchange tracks ordered address chain and emits enable-send receiver from sibling state | token messages from lower and higher sender addresses followed by no-message ticks | address chain remains ordered and one emitted enable-send uses broadcast receiver path |
