...
| `automation_component_debug_trace_request_resolves_hierarchical_rule_link` | debug request resolves hierarchical rule-name path without explicit `rules` segment | topic `$MONITOR/automation/ground/livingroom/roller/UpMorning/debug` with rule stored under `ground/livingroom/roller/rules/UpMorning` | publishes trace message with payload `triggered` and reason containing lookup success marker |
| `automation_component_debug_trace_raw_payload_escapes_multiline_reason_entries` | debug trace payload keeps reason chain parseable when expressions contain line breaks | rule check contains multiline script and debug request on matching rule | published trace raw payload contains escaped `\\n`, reason entries stay non-empty, and trace includes human-readable check reason |
