...
| `run_http_mqtt_interface_client_serves_endpoints_logs_real_broker_forward_publish` | publish flow must log exactly at broker-forward point with resolved MQTT message fields | POST `/publish` and `/publish.php` with compatibility payload | log contains explicit broker-forward entry including `topic=...` |
