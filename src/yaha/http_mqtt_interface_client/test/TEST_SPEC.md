...
| `http_mqtt_interface_component_serves_endpoints_logs_publish_and_stops_on_signal` | runtime HTTP contract and ingress logging for browser clients via IMqttComponent coupling | GET/PUT/POST/OPTIONS requests to `/publish`, `/publish.php`, `/pubrel` | responses include CORS headers, publish requests are logged, preflight returns `204`, and shared outgoing message log contains `event=broker_publish_ack` with publish-ingress reason `Request by User` |
