...
| `history_single_compression_keeps_total_entry_count` | Single-entry compression must preserve total message count | alternating values with single-entry-only config and stepwise updates | after each update: `1 + history.size() == inserted_messages` |
| `history_time_value_compression_keeps_total_entry_count` | TimeValue compression must preserve total message count | same reason with changing values and disabled further compression | after each update: `1 + history.size() == inserted_messages` |
| `history_time_compression_keeps_total_entry_count` | Time compression must preserve total message count | identical values with irregular timestamps to avoid interval conversion | after each update: `1 + history.size() == inserted_messages` |
| `history_interval_compression_keeps_total_entry_count` | Interval compression must preserve total message count | identical values with regular timestamps and low conversion threshold | after each update: `1 + history.size() == inserted_messages` |
| `history_time_to_interval_transition_keeps_total_entry_count` | Time-to-interval split must preserve total message count | sequence that first builds time/timeValue then converts regular tail to interval | after each update: `1 + history.size() == inserted_messages` |
| `history_multi_reason_same_device_timestamp_adds_exactly_one_logical_entry_per_update` | Reproduce client payload duplication pattern with two reasons and fixed device timestamp | repeated updates on same topic/value with reasons `received from arduino` + `received by broker` while first reason timestamp stays equal | logical count grows exactly by one per update and no duplicate timestamps are generated inside one update step |
| `history_stale_first_reason_timestamp_does_not_collapse_new_updates` | Detect regression where stale first reason timestamp makes new updates look old | repeated updates with constant first reason timestamp and newer broker timestamp in later reason entries | projected current+history timestamps remain distinct per update and do not collapse to one old time |
| `history_distinct_first_reason_timestamps_keep_distinct_projected_times` | Distinct first reason timestamps must map to distinct projected times | repeated updates with different parseable `reason[0].timestamp` values | projected current+history timestamps remain distinct and increase by one logical time per update |
...
| `get_nodes_returns_only_changed_nodes_for_required_snapshot_topics` | Snapshot diff mode follows legacy JS required-node iteration | snapshot with same and different topics | only changed nodes for provided snapshot topics are returned |
...
| `run_and_close_log_compression_stats_for_start_and_stop` | Lifecycle stats observability for startup/shutdown | run then close with stdout capture | output contains `message_store[stats]` headers for start/stop plus multiline stat rows (`name : value`) |
| `run_writes_loaded_state_replay_dump_and_logs_post_restore_messages` | Reproduction recorder writes startup dataset and post-restore incoming stream | preloaded snapshot + replay file paths + one incoming message after run | loaded-state file is written in envelope JSONL and incoming file contains only post-restore message |
...
| `http_post_store_sensor_payload_uses_topic_and_query_flags` | sensor.php-compatible POST fields drive section query | POST body with `topic`, `history`, `reason`, `time`, `levelAmount` | status 200 and section query applies topic + flags |
...
| `http_post_store_sensor_payload_nodes_applies_explicit_level_amount` | Explicit `levelAmount` also constrains snapshot nodes mode | POST body with `topic`, `levelAmount`, and nested snapshot nodes | status 200 and only changed nodes within requested depth are returned |
| `http_post_store_sensor_payload_empty_nodes_uses_section_query` | sensor.php-compatible empty `nodes` must not force diff mode | POST body with `topic`, flags and `nodes: []` | status 200 and section query uses topic/history/reason/time filters |
| `http_post_store_sensor_payload_accepts_json_boolean_flags` | sensor.php-compatible flags accept JSON booleans and legacy lower-case level key | POST body with `history: true`, `reason: false`, `time: false`, `levelamount` and `nodes: []` | status 200 and section query honors flags and level |
