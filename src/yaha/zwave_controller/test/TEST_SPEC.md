...
| `same_action_replaces_pending_entry` | Repeated identical action replaces older pending command entry | track same target+value twice with different reasons, then matching value change | outbound message uses only latest reasons |
