...
| `on_value_refreshed_publishes_mapped_switch_as_on_off` | Refreshed callback event must also publish current switch state | node/value refreshed event with mapped switch and numeric on payload | publishes mapped topic with value `on` |
