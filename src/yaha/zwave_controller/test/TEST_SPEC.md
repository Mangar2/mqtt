...
| `notification_callback_maps_all_codes_and_uses_unknown_topic_fallback` | Notification callback should map each notification code and fallback topic for unknown nodes | invoke `onNotification` with all enum values on unmapped node id | publishes to `/$MONITORING/zwave/unknown node <id>` with expected text values |
...
| `on_value_changed_without_mapping_falls_back_to_monitoring_topic` | Missing mapping should fallback to monitoring publish path | value change event on unmapped node | publishes to `$MONITORING/zwave/<nodeId>` |
