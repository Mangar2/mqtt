...
| `expression_tokenizer_recognizes_comparators` | all comparison and arithmetic separator operators tokenized correctly | `a=b and c!=d and e<>f and g>=h and i<=j and k>l and m<n and o*p=q / r` | vector contains exact operator tokens |
| `expression_tokenizer_keeps_topic_and_splits_division_operator` | slash in topic path is preserved while arithmetic division is split as operator | `outdoor/garden/weather2/sensor/pressure / 10` | token vector is `outdoor/garden/weather2/sensor/pressure`, `/`, `10` |
...
| `internal_variables_reports_detailed_reason_when_sun_event_is_undefined` | sun-event error diagnostics include actionable cause and context | polar coordinates where sun-event crossing is undefined | thrown error text contains event name, `cause`, `cosHourAngle`, coordinate values, and UTC date |
| `internal_variables_keeps_twilight_keys_defined_for_central_europe_summer` | central-Europe summer must not fail full internal variable generation when astronomical twilight is undefined | coordinates `longitude=8.205`, `latitude=49.912` on `2026-06-02` | calculation succeeds and keeps sunrise/sunset plus astronomical twilight keys defined |
...
| `expression_parser_gives_mul_div_higher_precedence_than_add_sub` | arithmetic precedence correctness for `*` and `/` against `+` and `-` | `1 + 2 * 3 - 8 / 4` | parser succeeds and AST nests multiply/divide under add/sub |
...
| `rules_tree_json_reader_reports_invalid_escape_and_parses_unicode_escape` | JSON reader escape handling | one payload with invalid `\q` and one payload with valid `\u0041` | invalid escape fails with error, unicode escape parses successfully to string `A` |
...
| `expression_evaluator_helpers_parse_double_and_numeric_like_variants` | numeric helper parsing and whitespace handling | valid double text, partial token text, string runtime values incl. trimmed/empty | valid parse succeeds, partial token fails, numeric-like conversion matches expectations |
| `expression_evaluator_helpers_parse_time_text_and_iso_offsets` | time helper ISO/time parsing branches | `HH:MM:SS`, `HH:MM`, valid ISO with `Z` and signed offsets | parsed seconds are returned and equivalent instants map to equal local time-of-day |
| `expression_evaluator_helpers_reject_invalid_iso_forms` | ISO parser rejection branches | malformed ISO strings (fraction without digits, missing timezone colon, invalid date, trailing token, invalid offset digit) | each invalid input returns empty optional |
| `expression_evaluator_helpers_format_and_type_rendering` | value text/type rendering including map and time branches | runtime values of string, number, bool, time point, map | `valueToString`/`valueTypeToString` return expected display strings and type names |
| `expression_evaluator_helpers_bool_equals_and_error_cause` | bool coercion, equality, operator symbol, undefined-cause rendering | mixed runtime values and undefined variable names | `toBool`/`evalEquals`/`relationOperatorSymbol`/`appendUndefinedCause` follow expected legacy semantics |
| `expression_evaluator_supports_numeric_multiplication_and_division` | numeric `*` and `/` operations | expressions `6 * 7` and `42 / 10` | successful evaluation with numeric results `42` and `4.2` |
| `expression_evaluator_supports_topic_division` | division expression with slash-based topic variable | expression `outdoor/garden/weather2/sensor/pressure / 10` and topic value in variable map | successful evaluation with numeric result and used variable tracking |
| `expression_evaluator_reports_division_by_zero` | arithmetic safety for division by zero | expression `10 / 0` | evaluation fails and errors include division-by-zero message |
...
| `expression_evaluator_can_evaluate_program_from_rules_fixture` | fixture-like expression runtime with local wall-clock semantics | expression shape from converted `test/rules.json` with locally constructed `/time` and `/sunrise` values | returns expected branch output independent of host timezone offset |
...
| `single_rule_processor_maps_time_expression_value_to_iso_string` | value expression returns internal time variable | rule object with `value` expression `"/time"` and `/time` as runtime `time_point` | processing succeeds and emitted message value is ISO-8601 UTC string |
...
| `single_rule_processor_trace_includes_reasoned_check_and_value` | trace output contains human-readable check/value reasons with variable snapshots | rule with check and value expressions and matching variable context | trace contains `check reason=` and `value reason=` entries and includes `check input=<name> type=<...> value=<...>` entries |
