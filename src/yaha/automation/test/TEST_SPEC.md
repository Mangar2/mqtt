...
| `single_rule_processor_adds_trace_reason_to_emitted_message_when_program_executed` | rule output message contains full evaluation trace in reason when check/value programs run | triggered rule with check and value expression programs | emitted message has non-empty reason chain with check/value trace entries |
...
| `rules_tree_processor_emitted_messages_include_evaluation_trace_reason` | automation tree output propagates evaluation trace to message reason | rule tree with one triggered expression-based rule | emitted message reason is non-empty and contains finish trace marker |
