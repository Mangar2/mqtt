...
| `value_service_logs_incoming_and_outgoing_messages` | runtime logging for incoming and sent messages via shared message-log service | one `/set` input with active publish callback and logging flags enabled | stdout contains one `value_service <-` and one `value_service ->` structured line (asserted via `yaha::test::messageLogLinePrefix`) |
...
| `value_service_runtime_config_falls_back_on_invalid_subscribe_qos` | invalid qos parser path | ini with `valueservice.subscribeQoS=9` | load succeeds and keeps default QoS |
| `value_service_runtime_config_falls_back_on_invalid_filestore_use` | invalid bool parser path | ini with `filestore.use=maybe` | load succeeds and keeps default `use` value |
| `value_service_runtime_config_falls_back_on_invalid_filestore_retry_interval` | filestore startup retry interval range validation | ini with `filestore.startupRetryIntervalSeconds=0` | load succeeds and keeps default retry interval |
| `value_service_runtime_config_falls_back_on_invalid_valueservice_logging_bools` | invalid message-log bool parser path | ini with `valueservice.logIncomingMessages=maybe`, `logOutgoingMessages=maybe`, `logReason=maybe` | load succeeds and keeps default logging flags |
| `value_service_publish_throw_logs_out_fail_without_false_success` | callback throw path must not emit success outbound line | callback throws and one `/set` message triggers publish | stderr contains structured outgoing `event=publish_failed` line and no success outgoing line for same topic |
| `value_service_publish_result_failure_logs_category` | explicit callback failure result path logs structured category | callback returns `PublishResult::fail(AckTimeout)` and one `/set` message triggers publish | stderr contains `event=publish_failed` and `category=ack_timeout` |
