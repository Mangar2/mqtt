...
| `value_service_logs_incoming_and_outgoing_messages` | runtime logging for incoming and sent messages via shared message-log service | one `/set` input with active publish callback and logging flags enabled | stdout contains one structured incoming and one structured outgoing line with `component="value_service"` |
