...
| `serial_device/` | serialdevice phase 2 | SerialDevice domain contract, deterministic subscription derivation, internal serial message model, chunked parser parity, and wire serializer parity based on legacy interface behavior including receiver-map fallback quirks and `$SYS/serialdevice/#` system subscription. |
