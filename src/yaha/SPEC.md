...
|--------------|------|-------------|
| `message/`   | 1    | Universal YAHA Message value type plus shared YAHA envelope payload parser/builder utilities and unified message-flow logging service contracts used by clients and transports. |
...
| `zwave_controller/` | zwave phase 3 | ZWave controller adapter for callback event mapping, node/value cache management, and topic-based set routing via driver port abstraction, with extracted value/reason/topic utility components and a dedicated polling component for pending-command and periodic node-state polling orchestration. |
...
| `serial_device/` | serialdevice phase 4 | SerialDevice domain contract, deterministic subscription derivation, internal serial message model, chunked parser parity, wire serializer parity, mqtt<->serial mapping parity, and runtime lifecycle parity (keep-alive, send-queue pacing, retry/open behavior, trace control) based on legacy interface behavior including receiver-map fallback quirks and `$SYS/serialdevice/#` system subscription, plus YAHA-standard configurable incoming/outgoing message-flow logging and request/reply reason-chain correlation. |
| `serial_device_client/` | serialdevice phase 5 | Standalone SerialDevice INI mapping and runtime config types with deterministic fallback warnings for recoverable parse errors, shared message-log key mapping (`logIncomingMessages`, `logOutgoingMessages`, `logReason`), plus runtime composition wiring for SerialDevice component, serial transport, MQTT client, and signal-driven runtime wrapper. |
