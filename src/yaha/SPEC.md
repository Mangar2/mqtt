...
| `serial_device/` | serialdevice phase 3 | SerialDevice domain contract, deterministic subscription derivation, internal serial message model, chunked parser parity, wire serializer parity, and mqtt<->serial mapping parity based on legacy interface behavior including receiver-map fallback quirks and `$SYS/serialdevice/#` system subscription. |
