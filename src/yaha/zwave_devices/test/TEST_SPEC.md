...
| `topic_to_id_label_lookup_reports_missing_node_and_label` | Label lookup error branches | classless mapping with missing node and with missing label match | throws runtime_error for missing node and descriptive classId and type guidance when value-label lookup fails |
| `topic_to_id_defaults_multilevel_class_to_bool_when_type_missing` | Legacy default type fallback for class-configured rows without explicit type | mapping row with class `0x26` and omitted type | resolved id keeps class and uses type `bool` |
