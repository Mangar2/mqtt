...
| `topic_to_id_label_lookup_reports_missing_node_and_label` | Label lookup error branches | classless mapping with missing node and with missing label match | throws runtime_error for missing node and descriptive classId/type guidance when value-label lookup fails |
