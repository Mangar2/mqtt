...
| `publish_v1_request_sets_dup_header_from_message` | Publish v1 dup header derives from `Message.dup()`, not a separate options field | message constructed with `dup=true` | request `dup` header is `1` |
...
| `compat_publish_post_form_maps_to_publish_v1_defaults` | Browser compat form fields map to native publish v1 with defaults | `POST /publish` with `topic` and `value` fields | forwarded request has decoded topic plus default qos=1, retain=0, appended publish-ingress reason `Request by User`, and mapped message rawPayload equals mapped request payload |
...
| `compat_publish_json_body_reason_without_timestamp_is_accepted` | Reason entries without timestamp remain valid for legacy compatibility | `POST /publish` JSON body with `reason:[{"message":"..."}]` and no `timestamp` | response status is `204` and request is forwarded |
