...
| `source_adapter_ping_uses_receive_token` | Adapter uses receive token from connect handshake for keepalive ping | fake source broker enforces `token=recv-token` on `/pingreq` | connectAndSubscribe succeeds and ping succeeds without disconnect |
