...
| `handle_message_logs_incoming_with_full_reason_chain` | Message-flow logging via shared `buildMessageLogLine` shows the full reason chain, not just the first entry | message with two `addReason` entries passed to `handleMessage` | stdout contains `component="file_store" direction="incoming"` and both reason messages |
...
| `monitoring_publish_success_logs_outgoing_message` | Message-flow logging for successful monitoring publish via shared `buildMessageLogLine` | set callback + POST | stdout contains `component="file_store" direction="outgoing"` for the `$MONITOR/FileStore/changed` topic |
...
| `monitoring_publish_throw_logs_out_fail_without_false_success` | Monitoring callback throw must never emit success outbound log | callback throws and HTTP POST triggers monitoring publish | logs contain `file_store[out-fail]` and no matching `component="file_store" direction="outgoing"` for that event |
