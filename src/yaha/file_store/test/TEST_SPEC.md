...
| `handle_message_logs_incoming_with_full_reason_chain` | Message-flow logging via shared `buildMessageLogLine` shows the full reason chain, not just the first entry | message with two `addReason` entries passed to `handleMessage` | stdout contains `file_store <- topic/a :` (asserted via `yaha::test::messageLogLinePrefix`) and both reason messages |
...
| `monitoring_publish_success_logs_outgoing_message` | Message-flow logging for successful monitoring publish via shared `buildMessageLogLine` | set callback + POST | stdout contains `file_store -> $MONITOR/FileStore/changed :` (asserted via `yaha::test::messageLogLinePrefix`) |
...
| `monitoring_publish_throw_logs_out_fail_without_false_success` | Monitoring callback throw must never emit success outbound log | callback throws and HTTP POST triggers monitoring publish | logs contain `file_store[out-fail]` and no matching `file_store ->` marker for that event |
