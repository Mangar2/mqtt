...
InflightEntry make_entry(uint16_t pkt_id,
                         InflightDirection dir = InflightDirection::Outbound,
...
TEST_CASE("is_packet_id_in_use_false", "[store]") {
    InflightStore store;
    CHECK_FALSE(store.is_packet_id_in_use("c1", 99U, InflightDirection::Outbound));
}
