    ...
    return "\n".join(
        [
            ...
            *openzwave_cache_setup_lines,
            ...
            "fi",
            "",
            "# Capture state before touching the unit, so a service the admin",
            "# deliberately stopped stays stopped after this reinstall.",
            "service_previously_installed=0",
            f"if ${{SUDO}} systemctl list-unit-files --no-legend \"{service_name}\" 2>/dev/null | grep -q \"{service_name}\"; then",
            "  service_previously_installed=1",
            "fi",
            "service_was_active=0",
            f"if ${{SUDO}} systemctl is-active --quiet {service_name}; then",
            ...
            *[f"${{SUDO}} {line.strip()}" for line in openzwave_cache_chown_lines],
            "${SUDO} systemctl daemon-reload",
            f"${{SUDO}} systemctl enable {service_name}",
            "",
            "if [[ ${service_previously_installed} -eq 0 ]]; then",
            f"  ${{SUDO}} systemctl start {service_name}",
            f"  echo \"Started service {service_name} (first install).\"",
            "elif [[ ${service_was_active} -eq 1 ]]; then",
            f"  ${{SUDO}} systemctl restart {service_name}",
            f"  echo \"Restarted service {service_name} (was running).\"",
            "else",
            f"  echo \"Service {service_name} was stopped before install; leaving it stopped.\"",
            "fi",
            "",
...
def render_client_mqtt_settings_script() -> str:
    return "\n".join(
        [
            "#!/usr/bin/env bash",
            "set -euo pipefail",
            ...
            "  - Recursively scans all .ini files below --root",
            "  - Skips broker INI files (*/broker/*)",
            "  - Modifies only files containing a [mqtt] section",
            "  - Does not restart services",
            "",
            "Options:",
            "  --root <dir>         Deployment root (default: current working directory)",
            "  --set <key=value>    MQTT key/value to set (repeatable, required)",
            ...
            "  temp_file=\"$(mktemp)\"",
            "",
            "  awk -v target_key=\"${setting_key}\" -v target_value=\"${setting_value}\" '\''",
            "    BEGIN {",
            "      in_mqtt_section = 0",
            "      key_written = 0",
            "    }",
            "",
            "    function write_key_line() {",
            "      printf \"%s = %s\\n\", target_key, target_value",
            "    }",
            "",
            "    {",
            "      if ($0 ~ /^[[:space:]]*\\[[^]]+\\][[:space:]]*$/) {",
            "        if (in_mqtt_section == 1 && key_written == 0) {",
            "          write_key_line()",
            "          key_written = 1",
            "        }",
            "",
            "        normalized_section = tolower($0)",
            "        gsub(/[[:space:]]/, \"\", normalized_section)",
            "        in_mqtt_section = (normalized_section == \"[mqtt]\") ? 1 : 0",
            "",
            "        print $0",
            "        next",
            "      }",
            "",
            "      if (in_mqtt_section == 1) {",
            "        normalized_key_line = $0",
            "        sub(/^[[:space:]]*/, \"\", normalized_key_line)",
            "        sub(/[[:space:]]*=.*/, \"\", normalized_key_line)",
            "",
            "        if (tolower(normalized_key_line) == tolower(target_key)) {",
            "          if (key_written == 0) {",
            "            write_key_line()",
            "            key_written = 1",
            "          }",
            ...
            "fi",
            "",
            "if [[ ${#mqtt_settings[@]} -eq 0 ]]; then",
            "  echo \"At least one --set <key=value> is required.\" >&2",
            "  usage >&2",
            "  exit 1",
            "fi",
            "",
            "declare -a ini_files",
            "while IFS= read -r found_ini; do",
            "  ini_files+=(\"${found_ini}\")",
            "done < <(find \"${root_dir}\" -type f -name '*.ini' ! -path '*/broker/*' | sort)",
            "",
            "if [[ ${#ini_files[@]} -eq 0 ]]; then",
            "  echo \"No client INI files found below: ${root_dir}\"",
            "  exit 0",
            "fi",
            "",
            "scanned_count=0",
            ...
            "  if ! grep -Eq '^[[:space:]]*\\[mqtt\\][[:space:]]*$' \"${ini_file}\"; then",
            "    continue",
            "  fi",
            "",
            ...
            "    echo \"[DRY-RUN] would update: ${ini_file}\"",
            "    changed_count=$((changed_count + 1))",
            "    continue",
            "  fi",
            "",
            "  file_changed=0",
            "  for mqtt_setting in \"${mqtt_settings[@]}\"; do",
            "    setting_key=\"${mqtt_setting%%=*}\"",
            "    setting_value=\"${mqtt_setting#*=}\"",
            "",
            "    if [[ -z \"${setting_key}\" || \"${mqtt_setting}\" != *=* ]]; then",
            "      echo \"Invalid --set value: ${mqtt_setting} (expected key=value)\" >&2",
            "      exit 1",
            "    fi",
            "",
            "    if apply_mqtt_setting \"${ini_file}\" \"${setting_key}\" \"${setting_value}\"; then",
            "      file_changed=1",
            "    fi",
            "  done",
            "",
            ...
            "done",
            "",
            "echo \"Client INI files with [mqtt]: ${scanned_count}\"",
...
def main() -> int:
    ...
    try:
        ...
        client_mqtt_settings_script = output_dir / CLIENT_MQTT_SETTINGS_SCRIPT_NAME
        client_mqtt_settings_script.write_text(
            render_client_mqtt_settings_script(),
            encoding="utf-8",
        )
        set_executable(client_mqtt_settings_script)
